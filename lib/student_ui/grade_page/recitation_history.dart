...
class _RecitationHistoryPageState extends State<RecitationHistoryPage> {
  ...
  final Map<String, List<Map<String, dynamic>>> recitationData = {
    "March 2026": [
      {
        "date": "March 9, 2026",
        "items": ["10", "15"],
  ...
  void _showMonthPicker() {
    showModalBottomSheet(
      ...
      builder: (context) {
        List<String> months = recitationData.keys.toList();
        return Container(
  ...
  Widget build(BuildContext context) {
    final currentMonthRecords = recitationData[selectedMonth] ?? [];
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.end, // Pushes Month Selector to the Right
          children: [
            ...
            // Recitation List Container
            Container(
              width: double.infinity, // Ensures the box spans full width
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start, // Keeps content on the left
                children: [
                  if (currentMonthRecords.isEmpty)
                    const Center(
                      child: Padding(
                        padding: EdgeInsets.symmetric(vertical: 40),
                        child: Text("No recitation points found."),
                      ),
                    )
                  else
                    ...currentMonthRecords.map((data) => Column(
                      ...
                      children: [
                        ...
                        ...data['items'].map<Widget>((pts) => _recitationRow(pts)).toList(),
  ...
  Widget _recitationRow(String pts) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.verified_outlined, color: Colors.black54, size: 20),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              "You earn $pts pts for participating", 
